...
Pre-aligned CRAMs must be coordinate sorted, indexed, duplicate marked, aligned against the identical FASTA, and encoded with the configured `cram_version` (currently 3.0 for compatibility with pinned GATK 4.4/HTSJDK). Keep the CRAI at a standard samtools-discoverable path (`sample.cram.crai` or `sample.crai`); the manifest rejects arbitrary detached index paths. Preflight checks readable files, CRAM format, `SM`, sort order, contig lengths, reference compatibility, and index usability. Use `bam_sample` only when a reviewed CRAM header name intentionally differs from the manifest key.
...
For planning, allow roughly the combined size of all input alignments plus 2–3× the largest active alignment for temporary CRAM, caller, and sorting work. Mutect2 uses approximately 20 Mb callable shards with a configurable concurrency cap; Strelka requests 16 cores/32 GB for WGS; FASTQ alignment and duplicate marking request up to 32 cores/49 GB. Actual wall time and scratch scale with callable territory, depth, filesystem, and scheduler. Check `df -h .` and site quotas before a real cohort or benchmark.
