...
Pre-aligned CRAMs must be coordinate sorted, indexed, duplicate marked, and aligned against the identical FASTA. Keep the CRAI at a standard samtools-discoverable path (`sample.cram.crai` or `sample.crai`); the manifest rejects arbitrary detached index paths. Preflight checks readable files, `SM`, sort order, contig lengths, reference compatibility, and index usability. Use `bam_sample` only when a reviewed CRAM header name intentionally differs from the manifest key.
